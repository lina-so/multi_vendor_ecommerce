...
<div class="mb-5 ml-3">
    <a href="{{ route('dashboard.options.index') }}" class="btn btn-sm btn-outline-dark mr-2">back</a>
        ...
        <tbody>
            ...
            @forelse ($options as $option)
                <tr>
                    ...
                    {{-- restore --}}
                    <td>
                        <form action="{{ route('dashboard.options.restore', $option->id) }}" method="post">
                            @csrf
                            @method('put')
                    ...
                    {{-- forceDelete --}}
                    <td>
                        <form action="{{ route('dashboard.options.forceDelete', $option->id) }}" method="post">
                            @csrf
                            @method('delete')
            ...
            @empty
                <tr>
                    <td colspan="8" style="background-color:#007bff; color:white" class="text-center">No options
                        Difined</td>
                </tr>
            @endforelse
        </tbody>
